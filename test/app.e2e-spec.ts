...
describe('AppController (e2e)', () => {
  ...
  it('SA ban user', async () => {
    ...
    res = await request(server)
      .get(
        '/sa/users?sortBy=login&sortDirection=asc&pageSize=1&banStatus=banned',
      )
      .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' })
      .expect(200);
    expect(res.body).toStrictEqual({
  ...
  it('SA bind blog', async () => {
    await request(server)
      .put(
        '/sa/blogs/' +
          createResponseBlog_1.body.id +
          '/bind-with-user/' +
          createResponseUser_2.body.id,
      )
      .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' })
      .expect(204);
  });
  ...
  it('AUTH PUBLIC login with banned user', async () => {
    ...
    await request(server)
      ...
      .expect(204);
    //ban user
    const userId = (await service.returnUserByField('userforban')).id;
    await request(server)
  ...
  it('AUTH PUBLIC test change password', async () => {
    ...
    await request(server)
      ...
      .expect(400);
    const new_user = await service.returnUserByField('nastya1');
    await request(server)
      .post('/auth/new-password')
      .send({
        newPassword: 'qwerty11',
  ...
  it('PUBLIC get all blogs', async () => {
    res = await request(server).get('/blogs').expect(200);
    expect(res.body).toStrictEqual({
      page: 1,
      pageSize: 10,
      pagesCount: 1,
  ...
  it('BLOGGER ban user for blog', async () => {
    res = await request(server)
      ...
      .expect(404);
    const userId = (await service.returnUserByField('alina28')).id;
    await request(server)
  ...
  it('COMMENTS PUBLIC check for likes', async () => {
    ...
    await request(server)
      ...
      .auth(token_2.body.accessToken, { type: 'bearer' })
      .expect(204);
    const userId = (await service.returnUserByField('userthat')).id;
    await request(server)
      .put('/sa/users/' + userId + '/ban')
      .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' })
      .send({
  ...
  it('SA BLOGS ban blog and check banned blog', async () => {
    ...
    res = await request(server)
      .get(
        '/sa/users?searchLoginTerm=na&sortDirection=asc&sortBy=login&pageSize=15&pageNumber=1',
      )
      .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' })
      .expect(200);
    expect(res.body).toStrictEqual({
  ...
  it('PUBLIC SECURITY check for devices', async () => {
    ...
    await request(server)
      ...
      .expect(404);
    await request(server)
      .delete('/security/devices/' + res.body[0].deviceId)
      .set('Cookie', token_2.header['set-cookie'][0].split(';')[0])
      .expect(403);
    await request(server)
