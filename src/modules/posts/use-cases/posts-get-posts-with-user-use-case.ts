...
export class GetPostsWithUserUseCase
  implements ICommandHandler<GetPostsWithUserCommand>
{
  constructor(
  ...
  async execute(command: GetPostsWithUserCommand): Promise<PaginatedClass> {
    ...
    const paginatedItems = await this.queryRepository.postsMappingWithUser(
      items,
      userId,
    );
