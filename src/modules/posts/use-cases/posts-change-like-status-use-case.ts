...
export class ChangeLikeStatusPostsCommand {
  constructor(public requestType : string,public postId : string,public token : string) {
  }
}

@CommandHandler(ChangeLikeStatusPostsCommand)
export class ChangePostLikeStatusUseCase implements ICommandHandler<ChangeLikeStatusPostsCommand>{
  ...
  async execute (command : ChangeLikeStatusPostsCommand) : Promise<boolean>{
    ...
    const foundStatus = await this.likesRepository.findStatus(command.postId, userId)
