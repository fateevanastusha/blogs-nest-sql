...
export class GetPostsByBlogIdPostsCommand {
  constructor(public query : QueryModel,public blogId: string,public token : string | null) {
  }
}

@CommandHandler(GetPostsByBlogIdPostsCommand)
export class GetPostsByBlogIdUseCase implements ICommandHandler<GetPostsByBlogIdPostsCommand>{
  constructor(protected jwtService : JwtService,
              protected blogsRepository : BlogsRepository,
              protected queryRepository : QueryRepository,
              protected postsRepository : PostsRepository
              ) {}
  async execute (command : GetPostsByBlogIdPostsCommand) : Promise<PaginatedClass>{
    ...
    let total : number = await this.postsRepository.countPostsByBlogId(command.blogId)
    let pageCount = Math.ceil( total / command.query.pageSize)
    let items : PostModel[] = await this.queryRepository.paginatorForPostsWithBlog(command.query, command.blogId);
    if(blog.isBanned) {
      items = [];
      total = 0
      pageCount = 0
    ...
    return await this.queryRepository.paginationForm(pageCount, total, paginatedPosts, command.query)
  }
}