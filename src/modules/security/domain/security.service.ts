...
export class SecurityService {
  ...
  async getAllSessions(
    ...
    const sessions: RefreshTokensMetaModel[] | null =
      await this.securityRepository.getAllSessions(userId);
  ...
  async deleteAllSessions(refreshToken: string): Promise<boolean> {
    ...
    const status: boolean = await this.securityRepository.deleteAllSessions(
      idList.deviceId,
      idList.userId,
    );
