...
export class GetCommentsByBlogUseCase
  implements ICommandHandler<GetCommentsByBlogCommand>
{
  constructor(
  ...
  async execute(command: GetCommentsByBlogCommand): Promise<PaginatedClass> {
    ...
    const total = await this.commentsRepository.getCommentsCountByBlogOwnerId(
      userId,
    );
    const pageCount = Math.ceil(total / +command.query.pageSize);
    const items: CommentModel[] =
      await this.queryRepository.paginatorForCommentsByBlogOwner(
        command.query,
        userId,
      );
    const mappedItems = await Promise.all(
      items.map(async (a) => {
        const likes = (
          await this.likesRepository.getLikesInfoWithUser(userId, a.id)
        )[0];
        const postTitle = (await this.postsRepository.getPost(a.postId)).title;
        return {
          id: a.id + '',
          content: a.content,
          commentatorInfo: {
