...
export class GetCommentsByPostUseCase
  implements ICommandHandler<GetCommentsByPostCommand>
{
  constructor(
  ...
  async execute(command: GetCommentsByPostCommand): Promise<PaginatedClass> {
    ...
    const items: CommentModel[] =
      await this.queryRepository.paginatorForCommentsByPostId(
        command.query,
        command.postId,
      );
    let mappedComments;
    if (command.header) {
      ...
      mappedComments = await this.queryRepository.commentsMappingWithUser(
        items,
        userId,
      );
    } else {
    ...
    const total: number = await this.commentsRepository.countCommentsByPostId(
      command.postId,
    );
    const pageCount: number = Math.ceil(total / command.query.pageSize);
    const paginatedComments = await this.queryRepository.paginationForm(
      pageCount,
      total,
      mappedComments,
      command.query,
