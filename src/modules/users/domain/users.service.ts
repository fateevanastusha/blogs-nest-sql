...
export class UsersService {
  ...
  async getUsers(query: QueryModelUsers): Promise<PaginatedClass> {
    let total: number = await this.usersRepository.getUsersCount(
    ...
    const items: UserModel[] = await this.queryRepository.paginationForUsers(
      query,
    );
    //mapping items
    ...
    return this.queryRepository.paginationForm(
      pageCount,
      total,
      mappedItems,
      query,
