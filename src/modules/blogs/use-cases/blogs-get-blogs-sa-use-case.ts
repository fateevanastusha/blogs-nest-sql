...
export class GetBlogsSaUseCase implements ICommandHandler<GetBlogsSaBlogsCommand>{
  constructor(protected jwtService : JwtService,
              protected blogsRepository : BlogsRepository,
              protected queryRepository : QueryRepository
  ) {}
  async execute (command : GetBlogsSaBlogsCommand) : Promise<PaginatedClass>{
    let total = await this.blogsRepository.getBlogsCount(command.query.searchNameTerm)
    if(!total) total = 0
    const pageCount = Math.ceil( total / +command.query.pageSize)
    const items : BlogModel[] = await this.queryRepository.paginationForBlogsWithAdmin(command.query);
