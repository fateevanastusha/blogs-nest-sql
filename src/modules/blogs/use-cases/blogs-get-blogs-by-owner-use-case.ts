...
export class GetBlogsByOwnerUseCase
  implements ICommandHandler<GetBlogsByOwnerBlogsCommand>
{
  constructor(
  ...
  async execute(command: GetBlogsByOwnerBlogsCommand): Promise<PaginatedClass> {
    ...
    const total = await this.blogsRepository.getBlogsCountWithUser(
      command.query.searchNameTerm,
      userId,
    );
    const pageCount = Math.ceil(total / +command.query.pageSize);
    const items: BlogViewModel[] =
      await this.queryRepository.paginationForBlogsWithUser(
        command.query,
        userId,
      );
