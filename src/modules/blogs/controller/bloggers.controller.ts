...
export class BloggersController {
  ...
  async getBlogs(
    ...
    return await this.commandBus.execute(
      new GetBlogsByOwnerBlogsCommand(
        {
          pageSize: pageSize,
  ...
  async updateBlog(
    ...
    const token = req.headers.authorization!.split(' ')[1];
    return await this.commandBus.execute(
      new UpdateBlogBlogsCommand(blog, blogId, token),
    );
  }
  ...
  async updatePost(
    ...
    const token = req.headers.authorization!.split(' ')[1];
    return await this.commandBus.execute(
      new UpdatePostPostsCommand({ ...post, blogId: blogId }, postId, token),
    );
  }
  ...
  async deleteBlog(@Param('id') blogId: string, @Req() req: Request) {
    const token = req.headers.authorization!.split(' ')[1];
    return await this.commandBus.execute(
      new DeleteBlogBlogsCommand(blogId, token),
    );
  }
