...
export class AuthController {
  ...
  async loginRequest(@Req() req: any, @Res() res: any) {
    ...
    const tokenList: TokenList | null = await this.authService.authRequest(
      req.body.password,
      req.ip,
    ...
    if (tokenList) {
      ...
      res.cookie('refreshToken', tokenList.refreshToken, {
        httpOnly: true,
        secure: true,
      });
      res.send(token);
  ...
  async passwordRecoveryRequest(@Req() req: any) {
    const status: boolean = await this.authService.passwordRecovery(
      req.body.email,
    );
    if (status) return;
    else {
  ...
  async newRecoveryRequest(@Req() req: any) {
    const status: boolean = await this.authService.changePasswordWithCode(
  ...
  async confirmationRequest(@Req() req: any) {
    const status = await this.authService.checkForConfirmationCode(
      req.body.code,
    );
    if (!status) {
  ...
  async logoutRequest(@Req() req: any) {
    const status: boolean = await this.authService.logoutRequest(
  ...
  async refreshTokenRequest(@Req() req: any, @Res() res: any) {
    const title = req.headers['user-agent'] || 'unknown';
    const tokenList: TokenList = await this.authService.createNewToken(
      req.cookies.refreshToken,
      req.ip,
      title,
    );
    const token: AccessToken = { accessToken: tokenList.accessToken };
    res.cookie('refreshToken', tokenList.refreshToken, {
      httpOnly: true,
      secure: true,
    });
  ...
  async getInformation(@Req() req: any) {
    ...
    const user: UserModel | null =
      await this.authService.getInformationAboutCurrentUser(token);
    if (user) {
      const currentUser = {
        email: user.email,
        login: user.login,
