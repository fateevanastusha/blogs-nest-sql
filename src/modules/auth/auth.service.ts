...
export class AuthService {
  ...
  async authRequest(
    ...
    //GET TOKENS
    const refreshToken: RefreshToken = await this.jwtService.createJWTRefresh(
      user.id,
      deviceId,
    );
    const accessToken = await this.jwtService.createJWTAccess(user.id);
    //GET DATE
    const date: string | null = await this.jwtService.getRefreshTokenDate(
  ...
  async createNewToken(
    ...
    const accessToken: AccessToken = await this.jwtService.createJWTAccess(
      userId,
    );
    const newRefreshToken: RefreshToken =
      await this.jwtService.createJWTRefresh(userId, deviceId);
    const date: string | null = await this.jwtService.getRefreshTokenDate(
  ...
  async checkForConfirmationCode(confirmationCode: string): Promise<boolean> {
    const statusOfCode: boolean =
      await this.usersRepository.checkForConfirmationCode(confirmationCode);
    if (!statusOfCode)
      throw new BadRequestException({ message: ['code is wrong'] });
    const statusOfConfirmed: boolean =
      await this.usersRepository.checkForConfirmedAccountByEmailOrCode(
        confirmationCode,
      );
    if (statusOfConfirmed)
      throw new BadRequestException({ message: ['code is confirmed'] });
    const status = await this.usersRepository.changeConfirmedStatus(
      confirmationCode,
    );
    return status;
  }
  ...
  async emailResending(email: string): Promise<boolean> {
    //check email
    const user: UserModel[] = await this.usersRepository.returnUserByEmail(
      email,
    );
    if (user.length === 0) {
    ...
    const statusOfConfirmed: boolean =
      await this.usersRepository.checkForConfirmedAccountByEmailOrCode(email);
    if (statusOfConfirmed) {
      return false;
    }
    const confirmationCode: string = (+new Date()).toString();
